defmodule B1Web.HangmanView do
  ...
  def continue_of_try_again(conn, status) when status in [:won, :lost] do
    button("Try again", to: Routes.hangman_path(conn, :new))
  end

  def continue_of_try_again(conn, _) do
  ...
  defdelegate figure_for(truns_left), to: B1Web.HangmanView.Helpers.FigureFor
end
